  ...
  return (
    <div className="step text-gray-700">
      ...
      <div className="flex flex-inline justify-between items-center">
        <div
          className="font-bold cursor-pointer hover:scale-105"
          onClick={() => setIsStep((oldValue: boolean) => !oldValue)}
        >
          Accounts
        </div>
            ...
            btns={[
              {
                ...
                onClick: async () => {
                  ...
                  const failedAccounts = execution.accounts?.filter(
                    (acc) => acc.status === EStatuses.FAILED
                  );
              ...
              },
              {
                ...
                onClick: async () => {
                  ...
                  const uncompletedAccounts = execution.accounts?.filter(
                    (acc) => !notStatuses.includes(acc.status)
                  );
                  ...
                  if (uncompletedAccounts?.length) {
                    ...
                    const res = await apiMethodRequest(
                      ...
                      "PUT",
                      {
                        accounts: uncompletedAccounts,
                        status: EStatuses.STOPED,
                      }
                    );
                    console.log("res", res);
                    router.refresh();
              ...
              },
              {
                ...
                onClick: async () => {
                  ...
                  const stoppedAccounts = execution.accounts?.filter((acc) =>
                    isStatus.includes(acc.status)
                  );

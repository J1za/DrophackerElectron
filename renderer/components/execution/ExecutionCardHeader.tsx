...
export function ExecutionCardHeader({ execution }: { execution: IExecution }) {
  ...
  return (
    <div className="relative card-header flex flex-inline justify-between items-center">
      ...
      <BtnStatusColor
        ...
        btnClass="absolute top-0 left-1/2 transform -translate-x-1/2 "
      />
          ...
          },
          {
            ...
            onClick: async () => {
              ...
              const res = await apiMethodRequest(
                ...
                "POST",
                { id: execution.id, status: EStatuses.IN_PROGRESS }
              );
              console.log(res);
              setBools.inProgress(false);
              router.refresh();

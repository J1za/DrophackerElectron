...
import { pairFor } from "./utils/pair";
...
export default async function removeLiquidityETH(
  ...
  switch (action.name) {
    ...
    case EActions.CHECK_ALLOWANCE:
      ...
      // # Check Allowance
      const allowance = await getERC20Allowance(provider, args.lpToken.address, args.address, args.router); // prettier-ignore
    ...
    case EActions.APPROVE_ERC20:
      if (action.name === EActions.APPROVE_ERC20) {
        ...
        // # Approve erc20 Tokens
        const approveTx = await approveERC20(wallet, args?.lpToken.address, args?.router, args?.amountOfPoolToken, { gasPrice }); // prettier-ignore
...
async function prepareArgs(
  ...
  lpToken = await getLPCoin(ECoin.WETH, stepArgs.token.value, chain, EProtocols.MUTE); // prettier-ignore
  console.log("lpToken", lpToken);
  // "If lpToken was not Found, need to create lPToken.
  if (!lpToken) {
    const _provider = await initialiseProvider(undefined, provider.url);

    const lpAddress = await pairFor(tokenETH?.address!, token?.address!, router, _provider, false); // prettier-ignore
    ...
    lpToken = await createLpCoin(lpAddress, ECoin.WETH, stepArgs.token.value, chain, EProtocols.MUTE, _provider); // prettier-ignore
  }

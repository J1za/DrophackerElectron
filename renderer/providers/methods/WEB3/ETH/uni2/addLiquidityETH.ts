...
export default async function addLiquidityETH(
  ...
  switch (action.name) {
    ...
    case EActions.CHECK_ALLOWANCE:
      ...
      } else action = await completeActionAndNext(action, EActions.ADD_LIQUIDITY); // 'NEXT STEP
    ...
    case EActions.APPROVE_ERC20:
      if (action.name === EActions.APPROVE_ERC20) {
        ...
        action = await completeActionAndNext(action, EActions.WAIT_APPROVE); // 'WAIT APROVE
      }
    ...
    case EActions.ADD_LIQUIDITY:
      ...
      // `Check Eth Balance
      await checkBalanceERC20(provider, args.address, args.amountToken, args.token); // prettier-ignore

...
export default async function swapTokensForEth(
  stepState: StepState,
  stepArgs: IProps
) {
  ...
  switch (action.name) {
    ...
    case EActions.SWAP:
      ...
      // `Check Eth Balance
      await checkBalanceERC20(provider, args.address, args.amountIn, args.tokenIn); // prettier-ignore
      ...
      console.log("ARGS FOR SWAP:", minAmountOut.toString(), [args.tokenIn.address, args.tokenOut.address], wallet.address, deadline, [false], { value: args.amountIn }); // prettier-ignore

      // *** Make Swap
      const tx = await routerInstance.swapExactTokensForETH(
...
async function prepareArgs(
  ...
  // get WETH address
  const tokenOut = await getCoin(ECoin.WETH, chain);
  // routerAddress
